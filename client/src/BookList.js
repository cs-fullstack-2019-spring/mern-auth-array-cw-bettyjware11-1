...
class BookList extends Component{
    ...
    componentDidMount() {
        // Fetch the favoriteBookData whether they are logged in or not
        this.fetchUserFavoriteBookData();
    }
    ...
    signInUser=(e)=>{
        ...
        // On the right side, use that data parameter with the .text function to change data to normal text.
            ...
            .then(response=>{if(response) {
                ...
                // Now that I have the user's username, I want to fetch the favoriteBookArray and save it as a state
                return this.fetchUserFavoriteBookData();
            }
    ...
    // Fetch the signed in user's collection and save it in favoriteBooks
    fetchUserFavoriteBookData(){
        // Fetches the '/grabFavoriteBook' route in the users.js group as a GET method
        fetch('/users/grabFavoriteBook')
        // data on the left side is the raw response data the server sent (res.send)
        // On the right side, use that data parameter with the .json function to change data to a readable JSON collection.
            ...
            // on the right side we're setting the favoriteBooks state as the server's response. Afterwards, we're running a the mappedFavoriteBookFunction to map out each favoriteBookArray entry styling.
            .then(response=> {
                return this.setState({favoriteBooks: response.favoriteBooks}, () => this.mappedFavoriteBookFunction())
            });
    }

    // Map the user's favoriteBooks into a the mappedFavoriteBooks state
    mappedFavoriteBookFunction(){
        // This variable will be the mapped Array saved in the state
        let mapArray = [];

        console.log(this.state.todos);
        // Create a temporary array until you know if the favoriteBookArray is empty. If the favoriteBookArray has data, it will be saved in here. Otherwise, it would be undefined and break everything if we didn't save it as an empty array.
        let tempArray = [];
        // If there are items in the todoArray
        if(this.state.todos)
        // Save the array in the tempArray
            tempArray = this.state.favoriteBooks;

        // if the tempArray has something in it (being entries)
        if(tempArray.length>0) {
            // Map each item in the array to the return JSX below
            mapArray = this.state.favoriteBooks.map(
                (eachElement, index) => {
        ...
        // If the todoArray doesn't have anything in it
            ...
            console.log("no favorite books for " + this.props.logInfo.username);
            // Make mapArray empty
            mapArray = [];
        }
        // Save the state of the mapArray so you can display all of it's entries
        this.setState({mappedFavoriteBooks:mapArray});
    }
        ...
        return(
            <div>
                ...
                {this.props.logInfo.loggedIn?
                    // Render the username and the mapped FavoriteBook Items
                    (<div>
                        <h1>{this.props.logInfo.username}'s data</h1>
                        {this.state.mappedFavoriteBooks}
                    </div>):
